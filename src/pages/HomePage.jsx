            ...
            {dummyTestimonials.map((t, i) => (
              <motion.div key={t.id}
                ...
                <div className="flex items-center mb-1">
                  {[...Array(t.rating)].map((_, j) => <FiStar key={j} className="w-4 h-4 text-yellow-400 fill-current" />)}
                </div>
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 italic">"{t.text}"</p>
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center text-white font-bold text-sm">{t.avatar}</div>
                  <div>
